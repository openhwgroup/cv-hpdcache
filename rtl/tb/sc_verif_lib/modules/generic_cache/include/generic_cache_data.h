...
{
public:
    GenericCacheData(const std::string& name, size_t nways, size_t nsets, size_t nwords)
      : name_m(name)
    ...
    inline uint64_t getAddrSet(uint64_t addr) { return (addr / (words_m * 8)) % sets_m; }

    inline uint64_t getAddrWord(uint64_t addr) { return (addr / 8) % words_m; }
